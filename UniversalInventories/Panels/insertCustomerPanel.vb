Public Class insertCustomerPanel
	Private Sub btnInsert_Click(sender As Object, e As EventArgs) Handles btnInsert.Click
        If nameTxt.Text.Equals("") Or surnameTxt.Text.Equals("") Or numberTxt.Text.Equals("") Or emailTxt.Text.Equals("") Or addressTxt.Text.Equals("") Or suburbTxt.Text.Equals("") Or cityTxt.Text.Equals("") Or provinceTxt.Text.Equals("") Or postalTxt.Text.Equals("") Or passwordTxt.Text.Equals("") Then
            MessageBox.Show("Error! All fields must be filled in", "Add Customer", MessageBoxButtons.OK, MessageBoxIcon.Error)
        ...
        Else
            If emailValidation() = True And lblAlreadyInUse.Visible = False And cellValidation() = True And postalValidation() = True Then
                If passwordTxt.Text.Length >= 6 Then
                    CustomerTableAdapter1.Insert(nameTxt.Text, surnameTxt.Text, numberTxt.Text, emailTxt.Text, addressTxt.Text, suburbTxt.Text, cityTxt.Text, provinceTxt.Text, postalTxt.Text, passwordTxt.Text)
                    CustomerTableAdapter1.Fill(DsSystem1.CUSTOMER)
                    ...
                    MainForm.customerPanel.CUSTOMERTableAdapter.Fill(MainForm.customerPanel.DSSystem.CUSTOMER)
                    MainForm.displayPanel.Controls.Add(MainForm.customerPanel)
                Else
                    MessageBox.Show("Password must be at least 6 characters.", "Add Customer", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    passwordTxt.Focus()
                End If

            Else
                MessageBox.Show("Error! Please enter valid email address.", "Add Customer", MessageBoxButtons.OK, MessageBoxIcon.Error)
                emailTxt.Focus()
    ...
    Private Sub emailTxt_Leave(sender As Object, e As EventArgs) Handles emailTxt.Leave
        CustomerTableAdapter1.FillByEmail(DsSystem1.CUSTOMER, emailTxt.Text)
        If (DsSystem1.CUSTOMER.Rows.Count > 0) Then
